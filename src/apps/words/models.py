...
class WordQuerySet(models.QuerySet):
    ...
    def words_by_difficulty(self: models.Model, user: User, diffuculty: str) -> "WordQuerySet":
...
class Word(BaseModel):
    ...
    user = models.ForeignKey(User = get_user_model(), on_delete=models.CASCADE, related_name="words")
    word = models.CharField(max_length=64, unique=True, blank=False)
    translate = models.CharField(max_length=64, blank=True)
    context = models.TextField(verbose_name=_("context"), default="the context from which the word is taken")
    difficulty = models.CharField(verbose_name=_("difficulty"),max_length=1, choices=Difficulty.choices, blank=True)
